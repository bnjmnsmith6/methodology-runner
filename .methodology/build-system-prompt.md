...
## Project Context
Build a single HTML file with a large number that increments each time it's clicked.

## This RP's Objective
Build a single HTML file with a large number that increments each time it's clicked. Success criteria: Single HTML file loads in browser; Number displays prominently on page. Technical constraints: Single HTML file only, No external dependencies.
